...
class Header(object):
    ...
    def __init__(self, f=None):
        ...
        self.size = 0
        if f != None:
            sig = b"".join(unpack("4c", f.read(4)))
            if sig not in (b"DATA", b"ATAD"):
                raise TypeError("Invalid signature")
            self.version, self.size_, self.swaplen, self.num_item_types, self.num_items, self.num_raw_data, self.item_size, self.data_size = unpack(
                "8i", f.read(32)
            )
            if self.version != 4:
            ...
            self.size = sum(
                [
                    ...
                    (self.num_items + (2 * self.num_raw_data))
                    * 4,  # item offsets, data offsets, uncompressed data sizes
                ]
            )
...
class DataFileReader(object):
    def __init__(self, map_path):
        ...
        with open(self.map_path, "rb") as f:
            ...
            if item is not None:
                ...
                fmt = "{0}i".format(item_size // 4)
                version, author, map_version, credits, license, settings = item_data[
                    : items.Info.type_size
                ]
                if author > -1:
                    author = decompress(self.get_compressed_data(f, author))[:-1]
                if map_version > -1:
                    map_version = decompress(self.get_compressed_data(f, map_version))[
                        :-1
                    ]
                if credits > -1:
            ...
            for i in range(num):
                ...
                version, width, height, external, image_name, image_data = item_data[
                    : items.Image.type_size
                ]
                external = bool(external)
                name = decompress(self.get_compressed_data(f, image_name))[:-1].decode()
                data = (
                    decompress(self.get_compressed_data(f, image_data))
                    if not external
                    else None
                )
                image = items.Image(
                    external=external, name=name, data=data, width=width, height=height
                )
            ...
            for i in range(group_item_num):
                ...
                for j in range(num_layers):
                    item_size, item_data = self.get_item(
                        f, layer_item_start + start_layer + j
                    )
                    fmt = "{0}i".format(item_size // 4)
                    item_data = unpack(fmt, item_data)
                    ...
                    if type_ == LAYERTYPE_TILES:
                        ...
                        color = 4 * [0]
                        version, width, height, game, color[0], color[1], color[
                            2
                        ], color[
                            3
                        ], color_env, color_env_offset, image_id, data = item_data[
                            3 : type_size - 3
                        ]
                        name = None
                        if version >= 3:
                            name = (
                                ints_to_string(item_data[type_size - 3 : type_size])
                                or None
                            )
                        tile_list = []
                        ...
                        if game == 2:
                            ...
                            if version >= 3:
                                ...
                                if (
                                    len(item_data) > items.TileLayer.type_size
                                ):  # some security
                                    tele_data = item_data[items.TileLayer.type_size]
                                    if (
                                        tele_data > -1
                                        and tele_data < self.header.num_raw_data
                                    ):
                                        tele_data = decompress(
                                            self.get_compressed_data(f, tele_data)
                                        )
                                        for i in range(0, len(tele_data), 2):
                                            tele_list.append(tele_data[i : i + 2])
                                        tele_tiles = items.TileManager(
                                            data=tele_list, _type=1
                                        )
                            else:
                                # num of tele data is right after num of data for old maps
                                if (
                                    len(item_data) > items.TileLayer.type_size - 3
                                ):  # some security
                            ...
                            if version >= 3:
                                ...
                                if (
                                    len(item_data) > items.TileLayer.type_size + 1
                                ):  # some security
                                    speedup_data = item_data[
                                        items.TileLayer.type_size + 1
                                    ]
                                    if (
                                        speedup_data > -1
                                        and speedup_data < self.header.num_raw_data
                                    ):
                                        speedup_data = decompress(
                                            self.get_compressed_data(f, speedup_data)
                                        )
                                        for i in range(0, len(speedup_data), 4):
                                            speedup_list.append(speedup_data[i : i + 4])
                                        speedup_tiles = items.TileManager(
                                            data=speedup_list, _type=2
                                        )
                            else:
                                # num of speedup data is right after tele data
                                if (
                                    len(item_data) > items.TileLayer.type_size - 2
                                ):  # some security
                                    speedup_data = item_data[
                                        items.TileLayer.type_size - 2
                                    ]
                                    if (
                                        speedup_data > -1
                                        and speedup_data < self.header.num_raw_data
                        ...
                        version, num_quads, data, image_id = item_data[
                            3 : type_size - 3
                        ]
                        name = None
                        if version >= 2:
                        ...
                        layer = items.QuadLayer(
                            name=name, detail=detail, image_id=image_id, quads=quads
                        )
                        layers.append(layer)
            ...
            for i in range(num):
                ...
                envelope = items.Envelope(
                    name=name, version=version, channels=channels, envpoints=envpoints
                )
    ...
    def _get_item_size(self, index):
        ...
        if index == self.header.num_items - 1:
            return (
                self.header.item_size - self.item_offsets[index]
            ) - 8  # -8 to cut out type_and_id and size
    ...
    def get_item(self, f, index):
        ...
        if index < self.header.num_items:
            f.seek(
                self.header.size + self.item_offsets[index] + 8
...
class DataFileWriter(object):
    class DataFileItem(object):
        def __init__(self, type_, id_, data):
            self.type = type_
            self.id = id_
            self.data = pack("2i", (self.type << 16) | self.id, len(data)) + data
    ...
    def __init__(self, teemap, map_path):
            ...
            for layer in group.layers:
                if layer.type == "tilelayer":
                    ...
                    if layer.is_telelayer:
                        tile_data = len(datas)
                        datas.append(
                            DataFileWriter.DataFileData(
                                len(layer.tele_tiles.tiles) * b"\x00\x00\x00\x00"
                            )
                        )
                        ...
                        datas.append(
                            DataFileWriter.DataFileData(
                                len(layer.speedup_tiles.tiles) * b"\x00\x00\x00\x00"
                            )
                        )
        ...
        items_.append(
            DataFileWriter.DataFileItem(ITEM_ENVPOINT, 0, pack(fmt, *envpoints))
        )
        items_.sort()
        ...
        file_size = (
            header_size + item_types_size + offset_size + item_size + data_size - 16
        )
        swaplen = file_size - data_size

        # write file
        with open(map_path, "wb") as f:
            f.write(b"DATA")  # file signature
            header_str = pack(
                "8i",
                4,
                file_size,
